...
namespace wServer.realm
{
    public class ChatManager
    {
        ...
        public void Say(Player src, string text)
        {
            ...
            {
                Name = (src.Client.Account.Rank >= 6 ? "@[Owner] " : src.Client.Account.Rank >= 5 ? "![Dev] " : src.Client.Account.Rank >= 4 ? "#[Admin] " : src.Client.Account.Rank >= 3 ? "#[Mod] " : src.Client.Account.Rank >= 2 ? "[Spriter] " : src.Client.Account.Rank >= 1 ? "?[Donator] " : "") + src.Name,
                ObjectId = src.Id,
                Stars = src.Stars,
                BubbleTime = 10,
                Recipient = "",
