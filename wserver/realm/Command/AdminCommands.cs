...
namespace wServer.realm.commands
{
    ...
    {
        public Rank() : base("rank", Ranks.Owner) { }

        private const int MAX_RANK = 6;

        bool validRank(string rankId)
        {
            return rankId.All(c => c >= '0' && c <= '6');
        }

        String[] ranks = { "Donator", "Spriter", "Mod", "Admin","Dev",
            "Owner" };
        ...
        internal class PetSizeCommand : Command
    {
        ...
        internal class FeedPet : Command
        {
            public FeedPet() : base("maxpetboi", Ranks.Admin)
            { 
            }
            protected override bool Process(Player player, RealmTime time, string[] args)
            {
                Item food = player.Manager.GameData.Items[0xccc];

                for (int i = 0; i < 50; i++)
                {
                    player.Pet.Feed(food);
...
internal class TossEffCommand : Command
    ...
    {
        public SetGoldCommand() : base("gold", Ranks.Mod) { }
        protected override bool Process(Player player, RealmTime time, string[] args)
        {
    ...
    {
        public SetTokensCommand() : base("tokens", Ranks.Spriter) { }
        protected override bool Process(Player player, RealmTime time, string[] args)
        {
            if (string.IsNullOrEmpty(args[0]))
            {
    ...
    {
        public SetFameCommand() : base("fame", Ranks.Admin) { }
        protected override bool Process(Player player, RealmTime time, string[] args)
        {
            if (string.IsNullOrEmpty(args[0]))
            {
    ...
    {
        public BanCommand() : 
            base("ban", permLevel: Ranks.Admin)
        {
        }
    ...
    {
        public ListCommands() : base("commands", permLevel: Ranks.Donator) { }
